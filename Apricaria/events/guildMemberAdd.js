            ...
            .then((data) => {
                if (data.length > 0 && data[0].welcome === 1) {
                    const welcomeEmbed = embedConstructor.create(`Welcome to ${event.guild.name}!`, "We are glad to have you!", event.user, [
                        { "name": "About Me", "value": `I am <@${config.general.clientId}>, a Bèta General Purpose bot made by <@${config.general.authorSnowflake}>. I am in charge of the Level and Economy system, and keeping the server tidy. I also have fun commands, like 'Rock, Paper, Scissor' and utility commands to make managment easier.` },
