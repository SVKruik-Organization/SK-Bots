...
async function post(cost, product, quantity, type, interaction, remaining, guildSnowflake) {
    try {
        ...
        const response = await modules.database.query(`INSERT INTO purchase (snowflake, cost, product, quantity, type, date, remaining_bits, method, guild_snowflake) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?, "Virgo Discord Bot", ?); ${query}`, [interaction.user.id, cost, product, quantity, type, remaining, guildSnowflake])
