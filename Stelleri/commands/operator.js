    ...
    async execute(interaction) {
        try {
            ...
            if (targetMember && targetMember.id === interaction.user.id) return interaction.reply({
                content: `You cannot ${actionType} yourself ${actionType === "add" ? "to" : "from"} a team that you are already a member of via commands. To transfer ownership and use other advanced operations, please use the [Bot Commander](${config.urls.botCommanderWebsite}) application or the [website](${config.urls.website}).`,
                ephemeral: true
            });
