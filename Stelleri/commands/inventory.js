            ...
            modules.database.query('SELECT xp15, xp50, xp_active, xp_active_expiry as expiry FROM user_inventory WHERE snowflake = ?', [interaction.user.id])
                .then(async (data) => {
                    if (data.length === 0) {
                        ...
                        const dateDifference = data[0].expiry - date.getDate().today;
                        const remainingHours = Math.floor((dateDifference % 86400000) / 3600000);
                        const remainingMinutes = Math.round(((dateDifference % 86400000) % 3600000) / 60000);
                        return interaction.reply({
                            content: `You have already activated a XP-Booster (\`${data[0].xp_active}\`), and it expires in approximately \`${remainingMinutes === 60 ? remainingHours + 1 : remainingHours}\` hours and \`${remainingMinutes !== 60 ? remainingMinutes : 0}\` minutes.`,
                            ephemeral: true
                        });
                    }
                    ...
                    return interaction.reply({
                        content: 'Something went wrong while the required information. Please try again later.',
                        ephemeral: true
                    });
                });
